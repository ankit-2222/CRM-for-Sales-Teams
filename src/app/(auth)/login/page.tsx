  ...
  return (
    <div className="grid min-h-screen place-items-center p-4">
      <div className="w-full max-w-sm sm:max-w-md rounded-2xl bg-white p-6 sm:p-8 shadow-sm ring-1 ring-zinc-200">
        <h1 className="mb-6 text-center text-xl sm:text-2xl font-semibold text-zinc-900">Login</h1>
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <button disabled={loading} className="mt-2 w-full rounded-lg bg-blue-600 py-2 text-sm font-medium text-white transition hover:scale-[1.01] hover:bg-blue-500 disabled:opacity-60">{loading ? "Loading..." : "Login"}</button>
        </form>
        <p className="mt-4 text-center text-sm text-zinc-600">
          Don&apos;t have an account? <Link className="font-medium text-blue-600 hover:underline" href="/signup">Sign up</Link>
        </p>
      </div>
    </div>
  );
